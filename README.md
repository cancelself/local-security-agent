...
verdicts. Each run is persisted to `audits/` — a dated, evidence-backed log of
system state, findings, and remediation — so later runs diff against history
instead of starting cold. Audit logs are **gitignored** (local to the audited
machine); only [`audits/TEMPLATE.md`](audits/TEMPLATE.md) is tracked.
